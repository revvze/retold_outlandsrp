...
function PANEL:Move(newX, newY, givenInventory, bNoSend)
    ...
    LocalPlayer():EmitSound("willardnetworks/inventory/inv_move"..math.random(1,6)..".wav", nil, math.random(50.0,100.0), 0.1)
end
...
hook.Add("CreateMenuButtons", "ixInventory", function(tabs)
    ...
    tabs["inv"] = {
        ...
        Create = function(info, container)
            ...
            leftPanel:Dock(LEFT)
            leftPanel:SetWide(container:GetWide() / 2)
            ...
            rightPanel:SetWide(container:GetWide() / 2)
            rightPanel.Paint = function(this, w, h)
            end

            modelPanel = rightPanel:Add("ixModelPanel")
            modelPanel:Dock(FILL)
            modelPanel:DockMargin(10, 10, 10, 10)
            modelPanel:SetModel(LocalPlayer():GetModel())
            modelPanel:SetFOV(rightPanel:GetWide() / 12)
            modelPanel.Think = function()
                local ply = LocalPlayer()

                if ( modelPanel and modelPanel.Entity ) and ( IsValid(ply) and ply:Alive() ) then
