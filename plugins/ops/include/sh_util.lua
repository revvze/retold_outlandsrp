...
ix.command.Add("setarmor", {
    ...
    OnRun = function(self, ply, target, armor)
        if ( target ) then
            target:SetArmor(armor)
            ply:Notify("You have set "..target:Nick().."'s armor to "..armor..".")

            if ( target == ply ) then
                for v,k in pairs(player.GetAll()) do
                    if k:IsSuperAdmin() then
                        k:AddChatText(Color(135, 206, 235), "[ops] Moderator "..ply:SteamName().." set their armor to "..armor..".")
                    end
                end
            end
        else
