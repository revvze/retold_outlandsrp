...
function PLUGIN:PostPlayerDraw(k)
	...
	if k.Cosmetics then
		for a,b in pairs(k.Cosmetics) do
			...
			if b.drawdata.renderSetting then
				if ix.GetSetting(b.drawdata.renderSetting) then
					b:DrawModel()
				end
			else
				b:DrawModel()
	...
	local faceCos = k:GetLocalVar("COSFace") -- uses bone 6 face
	local hatCos = k:GetLocalVar("COSHead") -- uses bone 6 face
	local chestCos = k:GetLocalVar("COSChest") -- uses bone 1 spine

	if faceCos then
		if faceCos != (k.lastFace or -1) then
			MakeCosmetic(k, faceCos, "ValveBiped.Bip01_Head1", ix.Cosmetics[faceCos], 1)
			k.lastFace = faceCos
		end  
	...
	if hatCos then
		if hatCos != (k.lastHat or -1) then
			MakeCosmetic(k, hatCos, "ValveBiped.Bip01_Head1", ix.Cosmetics[hatCos], 2)
			k.lastHat = hatCos
		end  
	...
	if chestCos then
		if chestCos != (k.lastChest or -1) then
			MakeCosmetic(k, chestCos, "ValveBiped.Bip01_Spine2", ix.Cosmetics[chestCos], 3)
			k.lastChest = hatCos
		end  
...
hook.Add("SetupInventoryModel", "ixDrawCosmetics", function(panel)
	panel.lastFaceI = -1
	panel.lastHatI = -1
	panel.lastChestI = -1
