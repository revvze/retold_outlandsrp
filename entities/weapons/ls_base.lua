...
function SWEP:GetViewModelPosition( pos, ang )
	...
	if predicted then
		...
		if rd > 0.5 then
			c_look = Lerp(math.Clamp(ft * 5, 0, 1), c_look, 5 * c_move2)
		elseif rd < -0.5 then
			c_look = Lerp(math.Clamp(ft * 5, 0, 1), c_look, -5 * c_move2)
		else
	...
	if bob != 0 and c_move > 0 then
		...
		ang.y = ang.y + math.sin(ct * 7.4) * 1.2 * p
		ang.p = ang.p + math.sin(ct * 12.8) * 0.8 * p
		ang.r = ang.r + math.cos(ct * 5.4) * 0.3 * p
	end
...
function SWEP:DrawHUD()
	...
	if self.ScopePaint and self:GetIronsights() then
		self.ScopePaint(self)
		return
	end
