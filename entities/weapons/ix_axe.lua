    ...
    if ( tr ) then
        ...
        if ( IsValid(ent) and ent:GetClass() == "ix_mining_tree" ) then
            ...
            if ( ent:GetNWInt("ixHits", 0) >= 7 ) then
                if not ( self:GetOwner():GetCharacter():GetInventory():Add("firewood") ) then
                    self:GetOwner():Notify("You don't have the space to hold fire wood.")
                    ix.item.Spawn("firewood", tr.HitPos)
                else
