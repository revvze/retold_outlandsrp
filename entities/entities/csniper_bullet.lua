...
local speed = 10000 -- How quickly the sniper's bullet flies
local damage = 100 -- How much damage each bullet does
local force = 2500 -- The amount of force applied to everything the bullet passes through
...
function ENT:Initialize()
	...
	if SERVER then
		...
		local time = (pos - tr.HitPos):Length() / speed
...
if SERVER then
	...
	function ENT:Think()
		...
		local target = pos + (dir * (delta * speed))
		...
		if tr.Fraction != 1 then
			...
			if self.Impacts >= pen_max or (IsValid(ent) and (ent:IsNPC() or ent:IsPlayer())) or tr.HitTexture == "**displacement**" then
				ent:ForcePlayerDrop()
			...
			for i = 1, pen_steps do
				cursor = cursor + (dir * (pen_depth / pen_steps))
...
if CLIENT then
	function ENT:Think()
		...
		if not self.WhizDone then
			...
			local pos = self:GetPos()
			local dist, point = util.DistanceToLine(pos, pos + (self:GetAngles():Forward() * (delta * speed)), EyePos())
